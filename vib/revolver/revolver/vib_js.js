...
let innityAppsMaterials = [
  ...
  function innityAppsTriggerUrl(urlToLand) {
    // Demo Purposes
    if(innityAppsIsTriggerClickTagEnable === true) {
      innityAppsTrackerPauseVideo();
  ...
  // @deprecated Varaible to remove since we aren't support 2x 3x image.
  let innityAppsImageSuffix = '_2x';
  // @deprecated Varaible to remove since we aren't support 2x 3x image.
  let innityAppsImagesToLoad = 0;
  let innityAppsMaterialGeneratorVersion = '6.0.0';
  
  /**
   * Remove this function after confirm removal from MFV.
   * @deprecated Function to remove since we aren't support 2x 3x image.
   */
  function innityAppsDimensionRatioImageRename(elementType, materialEl) {
    if (elementType !== 'img') {
      return;
    }
    ...
    materialEl.src = materialEl.src.replace(/.png|.jpg|.jpeg/i, innityAppsImageSuffix + extension);
    innityAppsImagesToLoad++;
    materialEl.addEventListener('load', innityAppsImageLoadCompletedHandler);
  }
  
  function innityAppsGenerateMainContent(container, isDimensionRatio = false) {
    innityAppsGetDevicePixelRatio();
  
    for (let i = 0; i < innityAppsMaterials.length; i++) {
      innityAppsMaterialGenerator(innityAppsMaterials[i], container, isDimensionRatio);
    }
  
    innityAppsImageLoadDelay();
   ...
   * @deprecated Function to remove since we aren't support 2x 3x image.
   */
  function innityAppsGetDevicePixelRatio() {
    let ratio = 1;
   ...
   * @deprecated Function to remove since we aren't support 2x 3x image.
   */
  function innityAppsImageLoadCompletedHandler() {
    innityAppsImagesToLoad--;
  }
  
  function innityAppsImageLoadDelay() {
    if (innityAppsImagesToLoad > 0)
      setTimeout(innityAppsImageLoadDelay, 100);
    else
    {
   ...
   */
  function innityAppsMaterialGenerator(elementData, container, isDimensionRatio = false) {
    ...
    if (typeof elementData.childs !== 'undefined' && typeof elementData.childs === 'object') {
      for (let i = 0; i < elementData.childs.length; i++) {
        innityAppsMaterialGenerator(elementData.childs[i], materialEl, isDimensionRatio);
      }
    }
  
    // @deprecated Operation to remove since we aren't support 2x 3x image.
    if (isDimensionRatio === true) {
      innityAppsDimensionRatioImageRename(elementType, materialEl);
    }
  ...
  let innityAppsIsInvitationLoad = false;
  /**
  ...
  window.addEventListener('load', innityAppsInitCarolCreative, false);
  
  function innityAppsBindEvents() {
    window.addEventListener('message', innityAppsMessageHandler, false);
  }
  
  function innityAppsExpandedLandingCallback(index, isAutomation) {
    if (typeof expandedLandToSlideNumber === 'function') {
      expandedLandToSlideNumber(index, isAutomation);
    }
  }
  
  function innityAppsInitCarolCreative() {
    innityAppsSetupCreative();
    innityAppsPlatform = new InnityAppsMobilePlatform();
    innityAppsAd = new InnityAppsRevolver({
  //    autoRotateDirection: 'anti-clockwise', // Default is clockwise.
      expandedLoadedCallback: innityAppsExpandedLoadedCallback,
      expandedLandedCallback: innityAppsExpandedLandingCallback,
      invHiddenCallback: innityAppsPostExpandToProxy,
      invLandedCallback: innityAppsInvLandingCallback,
    });
    innityAppsBindEvents();
    innityAppsPostReadyToProxy();
    innityAppsDisableTouchToParent();
    innityAppsSetIOSWorkaround();
  }
  
  function innityAppsDisableTouchToParent() {
    document.getElementById('innity-apps-expanded-bg').addEventListener('touchmove', function(e) {
      e.stopPropagation();
      e.preventDefault();
    });
  
    // Demo Purposes
    document.getElementById('innity-apps-expanded-bg').addEventListener('mousemove', function(e) {
      e.stopPropagation();
      e.preventDefault();
    });
  }
  
  function innityAppsExpandedLoadedCallback() {
  ...
  function innityAppsInvLandingCallback(index, isAutomation) {
  ...
  function innityAppsMessageHandler(event) {
    ...
    switch (supportedMsg.action) {
      ...
      case 'shrink-banner':
        ...
        break;
      case 'resize':
        innityAppsAd.setParentWindowHeight(supportedMsg.data.windowHeight);
        break;
    }
  ...
  function innityAppsPostExpandToProxy() {
    parent.postMessage({owner: 'Innity', adType: 'innity-apps-mobile-carol', action: 'interaction', data: {interaction: 'expand-banner'}, version: innityAppsVersion}, '*');
  }
  
  function innityAppsPostReadyToProxy() {
    parent.postMessage({owner: 'Innity', adType: 'innity-apps-mobile-carol', action: 'adReady', data: {orientation: innityAppsIsOrientationEnable}, version: innityAppsVersion}, '*');
  }
  ...
  function innityAppsSetupCreative() {
    if (innityAppsIsInvitationLoad === true) {
      return;
    }
  
    innityAppsGenerateMainContent(document.getElementById('innity-apps-ad'));
    innityAppsIsInvitationLoad = true;
  }
  
  function InnityAppsRevolver(opts) {
    ...
    function initClass_() {
      if (innityAppsPlatform === null || innityAppsPlatform === undefined) {
        alert('Missing "InnityAppsMobilePlatform.js" JS file!');
        return;
      }
  
      invContainer_ = document.getElementById('innity-apps-invitation-ad');
      if (invContainer_ === null) {
        alert('Missing "innity-apps-invitation-ad" HTML element!');
        return;
      }
  
      touchSceneAreaEl_ = document.getElementById('innity-apps-invitation-scene');
      if (touchSceneAreaEl_ === null) {
        alert('Missing "innity-apps-invitation-scene" HTML element!');
        return;
      }
  
      invCarolContainer_ = document.getElementById('innity-apps-invitation-carol-container');
      if (invCarolContainer_ === null) {
        alert('Missing "innity-apps-invitation-carol-container" HTML element!');
        return;
      }
    ...
    function shrink() {
      invContainer_.classList.remove('innity-apps-hide');
    ...
    function bindEvents_() {
      ...
      touchSceneAreaEl_.addEventListener('touchstart', carolTouchHandler.touchStartHandler, {passive: true});
      touchSceneAreaEl_.addEventListener('touchmove', carolTouchHandler.touchMoveHandler, {passive: true});
  
      // Demo Purposes
      touchSceneAreaEl_.addEventListener('mousedown', carolTouchHandler.touchStartHandler, false);
      touchSceneAreaEl_.addEventListener('mousemove', carolTouchHandler.touchMoveHandler, false);
      touchSceneAreaEl_.addEventListener('mouseup', carolTouchHandler.dragRemoveHandler, false);
      touchSceneAreaEl_.addEventListener('mouseout', carolTouchHandler.dragRemoveHandler, false);
  
      invContainer_.addEventListener('transitionend', invTransitionEndHandler_);
  
      let carolItems = document.getElementsByClassName('innity-apps-carol-small');
    ...
    function calcSmallItemPosition_() {
      ...
      if (carolItems.length < 1) {
        alert('Missing "innity-apps-carol-small" HTML elements!');
        return;
      }
    ...
    function invTransitionEndHandler_(e) {
      if (e.target.id !== 'innity-apps-invitation-ad' || e.propertyName.toLowerCase() !== 'top') {
        return;
      }
    ...
    function iosWorkaround_() {
      ...
      document.getElementById('innity-apps-ad').classList.add('ios-workaround');
    ...
    function smallItemClickHandler_(e) {
      ...
      invContainer_.classList.add('innity-apps-hide');
      autoRotateOneRound_();
    ...
    function CarolBanner() {
      ...
      function initClass_() {
        bannerContainerWithBg_ = document.getElementById('innity-apps-expanded-bg');
        if (bannerContainerWithBg_ === null) {
          alert('Missing "innity-apps-expanded-bg" HTML element!');
          return;
        }
  
        bannerAd_ = document.getElementById('innity-apps-expanded-ad');
        if (bannerAd_ === null) {
          alert('Missing "innity-apps-expanded-ad" HTML element!');
          return;
        }
  
        bannerCarolContainer_ = document.getElementById('innity-apps-expanded-carol-container');
        if (bannerCarolContainer_ === null) {
          alert('Missing "innity-apps-expanded-carol-container" HTML element!');
          return;
        }
  
        bannerSceneTouchArea_ = document.getElementById('innity-apps-expanded-scene');
        if (bannerSceneTouchArea_ === null) {
          alert('Missing "innity-apps-expanded-scene" HTML element!');
          return;
        }
      ...
      function hide() {
        bannerContainerWithBg_.classList.add('innity-apps-hide');
      }
      ...
      function loadImage() {
        ...
        let imgEls = document.getElementsByClassName('innity-apps-banner-image');
      ...
      function show() {
        bannerContainerWithBg_.classList.remove('innity-apps-hide');
        resizeHandler_();
      ...
      function bindTouchEvents_() {
        let carolTouchHandler = new CarolTouchHandler(rotateToLeft_, rotateToRight_);
        bannerSceneTouchArea_.addEventListener('touchstart', carolTouchHandler.touchStartHandler, {passive: true});
        bannerSceneTouchArea_.addEventListener('touchmove', carolTouchHandler.touchMoveHandler, {passive: true});
  
        // Demo Purposes
        bannerSceneTouchArea_.addEventListener('mousedown', carolTouchHandler.touchStartHandler, false);
        bannerSceneTouchArea_.addEventListener('mousemove', carolTouchHandler.touchMoveHandler, false);
        bannerSceneTouchArea_.addEventListener('mouseup', carolTouchHandler.dragRemoveHandler, false);
        bannerSceneTouchArea_.addEventListener('mouseout', carolTouchHandler.dragRemoveHandler, false);
      }
  
      function bindTransitionEvents_() {
        let carolBigEls = document.getElementsByClassName('innity-apps-carol-big');
      ...
      function calcBigItemPosition_() {
        let carolItems = document.getElementsByClassName('innity-apps-carol-big');
        if (carolItems.length < 1) {
          alert('Missing "innity-apps-carol-big" HTML elements!');
          return;
        }
        ...
        for (let i = 0; i < totalItem_; i++) {
          let angle = (itemBaseAngle_ * i) + 3600;
          let curItem = carolItems[i];
          curItem.style.transform = `rotateY(${angle}deg) translateZ(${carolRadius}px)`;
          curItem.style.mozTransform = `rotateY(${angle}deg) translateZ(${carolRadius}px)`;
          curItem.style.webkitTransform = `rotateY(${angle}deg) translateZ(${carolRadius}px)`;
      ...
      function carolBigItemTransitionEndHandler_(e) {
        if (e.target.classList.contains('innity-apps-carol-big') !== true) {
          return;
        }
      ...
      function resizeHandler_() {
        ...
        let imgContainers = document.getElementsByClassName('innity-apps-carol-big');
    ...
    function CarolTouchHandler(toLeftCallback, toRightCallback) {
      ...
      // Demo Purposes
      this.dragRemoveHandler = dragRemoveHandler;
      ...
      function touchMoveHandler(e) {
        ...
        var _userClientX = (typeof e.touches !== 'undefined') ? e.touches[0].clientX : e.clientX;
        let xDiff = initX_ - _userClientX;
