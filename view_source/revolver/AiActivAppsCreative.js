...
let aiactivAppsIsInvitationLoad = false;
/**
...
let aiactivAppsVersion = '4.7.0';

window.addEventListener('load', aiactivAppsInitCarolCreative, false);

function aiactivAppsBindEvents() {
    window.addEventListener('message', aiactivAppsMessageHandler, false);
}

function aiactivAppsExpandedLandingCallback(index, isAutomation) {
    if (typeof expandedLandToSlideNumber === 'function') {
        expandedLandToSlideNumber(index, isAutomation);
    }
}

function aiactivAppsInitCarolCreative() {
    aiactivCheckHasTouchScreen();
    aiactivAppsSetupCreative();
    aiactivAppsPlatform = new AiActivAppsMobilePlatform();
    aiactivAppsAd = new AiActivAppsRevolver({
        //    autoRotateDirection: 'anti-clockwise', // Default is clockwise.
        expandedLoadedCallback: aiactivAppsExpandedLoadedCallback,
        expandedLandedCallback: aiactivAppsExpandedLandingCallback,
        invHiddenCallback: aiactivAppsPostExpandToProxy,
        invLandedCallback: aiactivAppsInvLandingCallback,
    });
    aiactivAppsBindEvents();
    aiactivAppsPostReadyToProxy();
    aiactivAppsDisableTouchToParent();
    aiactivAppsSetIOSWorkaround();
}

function aiactivAppsDisableTouchToParent() {
    document.getElementById('aiactiv-apps-expanded-bg').addEventListener('touchmove', function (e) {
        e.stopPropagation();
        e.preventDefault();
    });
}

function aiactivAppsExpandedLoadedCallback() {
    if (aiactivAppsIsBannerLoad === true) {
        return;
    }

    aiactivAppsIsBannerLoad = true;

    if (typeof aiactivAppsBannerLoaded === 'function') {
        aiactivAppsBannerLoaded();
    }
}
...
function aiactivAppsInvLandingCallback(index, isAutomation) {
    if (typeof invitationLandToSlideNumber === 'function') {
        invitationLandToSlideNumber(index, isAutomation);
    }
}
...
function aiactivAppsMessageHandler(event) {
    ...
    if (supportedMsg.version != aiactivAppsVersion) {
        if (aiactivAppsIsVersionDifferentWarned === false) {
            console.warn('Proxy and ad version is different! Proxy v' + supportedMsg.version + ' vs Ad v' + aiactivAppsVersion);
    ...
    switch (supportedMsg.action) {
        case 'shrink-banner':
            aiactivAppsAd.shrink();
            if (typeof expandedBannerClosed === 'function') {
                expandedBannerClosed();
            }
            break;
        case 'resize':
            aiactivAppsAd.setParentWindowHeight(supportedMsg.data.windowHeight);
            break;
    }

    switch (supportedMsg.data.eventType) {
        case 'deviceorientation':
            if (typeof deviceOrientation === 'function') {
                deviceOrientation(supportedMsg.data);
            }
            break;
        case 'orientationchange':
            if (typeof orientationChange === 'function') {
                orientationChange(supportedMsg.data);
            }
            break;
        default:
            break;
...
function aiactivAppsPauseAllVideo() {
...
function aiactivAppsPostExpandToProxy() {
    parent.postMessage({
        owner: 'AiActiv',
        adType: 'aiactiv-apps-mobile-carol',
        action: 'interaction',
        data: {
            interaction: 'expand-banner'
        },
...
function aiactivAppsPostReadyToProxy() {
    parent.postMessage({
        ...
        action: 'adReady',
        data: {
            orientation: aiactivAppsIsOrientationEnable
        },
        version: aiactivAppsVersion
    }, '*');
}
...
function aiactivAppsSetIOSWorkaround() {
    if (aiactivAppsPlatform.getOS() === 'ios') {
        aiactivAppsIosWorkAroundEl = document.getElementById('aiactiv-apps-ios-workaround');
        setInterval(aiactivAppsIosAppendText, 500);
    }
}

function aiactivAppsSetupCreative() {
    if (aiactivAppsIsInvitationLoad === true) {
        return;
    }

    aiactivAppsGenerateMainContent(document.getElementById('aiactiv-apps-ad'));
    aiactivAppsIsInvitationLoad = true;
}
...
function aiactivAppsDimensionRatioImageRename(elementType, materialEl) {
    ...
    materialEl.src = materialEl.src.replace(/.png|.jpg|.jpeg/i, aiactivAppsImageSuffix + extension);
...
function aiactivAppsGenerateMainContent(container, isDimensionRatio = false) {
    ...
    for (let i = 0; i < aiactivAppsMaterials.length; i++) {
        aiactivAppsMaterialGenerator(aiactivAppsMaterials[i], container, isDimensionRatio);
    }
...
function aiactivAppsImageLoadDelay() {
    if (aiactivAppsImagesToLoad > 0)
        setTimeout(aiactivAppsImageLoadDelay, 100);
    else {
 ...
 */
function aiactivAppsMaterialGenerator(elementData, container, isDimensionRatio = false) {
    ...
    if (typeof elementData.childs !== 'undefined' && typeof elementData.childs === 'object') {
        for (let i = 0; i < elementData.childs.length; i++) {
            aiactivAppsMaterialGenerator(elementData.childs[i], materialEl, isDimensionRatio);
        }
    }
...
(function (w) {
    var H = function () {
        return w.requestAnimationFrame || w.webkitRequestAnimationFrame || w.mozRequestAnimationFrame || function (a) {
            w.setTimeout(a, 1E3 / 60)
        }
    }(),
        d = w.jsmpeg = function (a, c) {
            c = c || {};
            this.progressive = !1 !== c.progressive;
            this.benchmark = !!c.benchmark;
            this.canvas = c.canvas || document.createElement("canvas");
            this.wantsToPlay = this.autoplay = !!c.autoplay;
            this.loop = !!c.loop;
            this.seekable = !!c.seekable;
            this.externalLoadCallback = c.onload || null;
            this.externalDecodeCallback = c.ondecodeframe || null;
            this.externalFinishedCallback = c.onfinished || null;
            this.customIntraQuantMatrix = new Uint8Array(64);
            this.customNonIntraQuantMatrix = new Uint8Array(64);
            this.blockData = new Int32Array(64);
            this.zeroBlockData = new Int32Array(64);
            this.fillArray(this.zeroBlockData, 0);
            !c.forceCanvas2D && this.initWebGL() ? this.renderFrame = this.renderFrameGL : (this.canvasContext = this.canvas.getContext("2d"), this.renderFrame = this.renderFrame2D);
            a instanceof WebSocket ? (this.client = a, this.client.onopen = this.initSocketClient.bind(this)) : this.load(a)
        };
    d.prototype.waitForIntraFrame = !0;
    d.prototype.socketBufferSize = 524288;
    d.prototype.initSocketClient = function () {
        this.buffer = new v(new ArrayBuffer(this.socketBufferSize));
        this.nextPictureBuffer = new v(new ArrayBuffer(this.socketBufferSize));
    ...
    d.prototype.decodeSocketHeader = function (a) {
        a[0] === A.charCodeAt(0) && a[1] === A.charCodeAt(1) && a[2] === A.charCodeAt(2) && a[3] === A.charCodeAt(3) && (this.width = 256 * a[4] + a[5], this.height = 256 * a[6] + a[7], this.initBuffers())
    };
    d.prototype.receiveSocketMessage = function (a) {
        var c = new Uint8Array(a.data);
        this.sequenceStarted || this.decodeSocketHeader(c);
        a = this.buffer;
        var b = this.nextPictureBuffer;
        b.writePos + c.length > b.length && (b.lastWriteBeforeWrap = b.writePos, b.writePos = 0, b.index = 0);
        b.bytes.set(c, b.writePos);
        b.writePos += c.length;
        for (c = 0; ;) {
            c = b.findNextMPEGStartCode();
            if (c === v.NOT_FOUND || b.index >> 3 > b.writePos) {
                b.index = Math.max(b.writePos - 3, 0) << 3;
                return
            }
            if (c === x) break
        }
        if (this.waitForIntraFrame) b.advance(10), b.getBits(3) === B && (this.waitForIntraFrame = !1, b.chunkBegin = b.index - 13 >> 3);
        else {
            this.currentPictureDecoded || this.decodePicture(D);
            c = b.index >> 3;
            if (c > b.chunkBegin) a.bytes.set(b.bytes.subarray(b.chunkBegin, c)), a.writePos = c - b.chunkBegin;
            else {
                a.bytes.set(b.bytes.subarray(b.chunkBegin, b.lastWriteBeforeWrap));
                var f = b.lastWriteBeforeWrap - b.chunkBegin;
                a.bytes.set(b.bytes.subarray(0, c), f);
                a.writePos = c + f
            }
            a.index = 0;
            b.chunkBegin = c;
            this.currentPictureDecoded = !1;
            H(this.scheduleDecoding.bind(this), this.canvas)
        }
    };
    d.prototype.scheduleDecoding = function () {
        this.decodePicture();
        this.currentPictureDecoded = !0
    };
    d.prototype.isRecording = !1;
    d.prototype.recorderWaitForIntraFrame = !1;
    d.prototype.recordedFrames = 0;
    d.prototype.recordedSize = 0;
    d.prototype.didStartRecordingCallback = null;
    d.prototype.recordBuffers = [];
    d.prototype.canRecord = function () {
        return this.client && this.client.readyState === this.client.OPEN
    };
    d.prototype.startRecording = function (a) {
        this.canRecord() && (this.discardRecordBuffers(), this.recorderWaitForIntraFrame = this.isRecording = !0, this.didStartRecordingCallback = a || null, this.recordedSize = this.recordedFrames = 0, this.recordBuffers.push(new Uint8Array([0, 0, 1, 179, this.width >> 4, (this.width & 15) << 4 | this.height >> 8, this.height & 255, 19, 255, 255, 225, 88, 0, 0, 1, 184, 0, 8, 0, 0, 0, 0, 1, 0])))
    };
    d.prototype.recordFrameFromCurrentBuffer = function () {
        if (this.isRecording) {
            if (this.recorderWaitForIntraFrame) {
                if (this.pictureCodingType !== B) return;
                this.recorderWaitForIntraFrame = !1;
                this.didStartRecordingCallback && this.didStartRecordingCallback(this)
            }
            this.recordedFrames++;
    ...
    d.prototype.discardRecordBuffers = function () {
        this.recordBuffers = [];
    ...
    d.prototype.intraFrames = [];
    d.prototype.currentFrame = -1;
    d.prototype.currentTime = 0;
    d.prototype.frameCount = 0;
    d.prototype.duration = 0;
    d.prototype.progressiveMinSize = 131072;
    d.prototype.fetchReaderPump = function (a) {
        var c = this;
        a.read().then(function (b) {
            c.fetchReaderReceive(a, b)
        })
    };
    d.prototype.fetchReaderReceive = function (a, c) {
        if (c.done) {
            if (this.seekable) {
                var b = this.buffer.index;
                this.collectIntraFrames();
                this.buffer.index = b
            }
            this.duration = this.frameCount / this.pictureRate;
            this.lastFrameIndex = this.buffer.writePos << 3
        } else this.buffer.bytes.set(c.value, this.buffer.writePos), this.buffer.writePos += c.value.byteLength, this.lastFrameIndex = this.findLastPictureStartCode(), !this.sequenceStarted && this.buffer.writePos >= this.progressiveMinSize ? (this.findStartCode(E), this.firstSequenceHeader = this.buffer.index, this.decodeSequenceHeader(), this.nextFrame(), this.autoplay && this.play(), this.externalLoadCallback && this.externalLoadCallback(this)) : this.sequenceStarted && this.wantsToPlay && !this.playing ? this.play() : this.sequenceStarted || (b = {
            loaded: this.buffer.writePos,
            total: this.progressiveMinSize
        }, this.gl ? this.updateLoaderGL(b) : this.updateLoader2D(b)), this.fetchReaderPump(a)
    };
    d.prototype.findLastPictureStartCode = function () {
        for (var a = this.buffer.bytes, c = this.buffer.writePos; 3 < c; c--)
            if (a[c] == x && 1 == a[c - 1] && 0 == a[c - 2] && 0 == a[c - 3]) return c - 3 << 3;
        return 0
    };
    d.prototype.load = function (a) {
        this.url = a;
        var c = this;
        if (this.progressive && w.fetch && w.ReadableByteStream) {
            var b = new Headers;
            b.append("Content-Type", "video/mpeg");
            fetch(a, {
                headers: b
            }).then(function (a) {
                var b = a.headers.get("Content-Length");
                a = a.body.getReader();
                c.buffer = new v(new ArrayBuffer(b));
                c.buffer.writePos = 0;
                c.fetchReaderPump(a)
            })
        } else {
            ...
            f.onreadystatechange = function () {
                f.readyState === f.DONE && 200 === f.status && c.loadCallback(f.response)
            };
            f.onprogress = this.gl ? this.updateLoaderGL.bind(this) : this.updateLoader2D.bind(this);
            f.open("GET", a);
            f.responseType = "arraybuffer";
            f.send()
        }
    };
    d.prototype.updateLoader2D = function (a) {
        a = a.loaded / a.total;
        ...
        f.fillRect(0, 0, c, b);
        f.fillStyle = "#fff";
        f.fillRect(0, b - b * a, c, b * a)
    };
    d.prototype.updateLoaderGL = function (a) {
        var c = this.gl;
        c.uniform1f(c.getUniformLocation(this.loadingProgram, "loaded"), a.loaded / a.total);
        c.drawArrays(c.TRIANGLE_STRIP, 0, 4)
    };
    d.prototype.loadCallback = function (a) {
        this.buffer = new v(a);
        this.seekable && (this.collectIntraFrames(), this.buffer.index = 0);
        this.findStartCode(E);
        this.firstSequenceHeader = this.buffer.index;
        this.decodeSequenceHeader();
        this.duration = this.frameCount / this.pictureRate;
        this.nextFrame();
        this.autoplay && this.play();
        this.externalLoadCallback && this.externalLoadCallback(this)
    };
    d.prototype.collectIntraFrames = function () {
        var a;
        for (a = 0; this.findStartCode(x) !== v.NOT_FOUND; a++) this.buffer.advance(10), this.buffer.getBits(3) === B && this.intraFrames.push({
            frame: a,
            index: this.buffer.index - 13
        });
        this.frameCount = a
    };
    d.prototype.seekToFrame = function (a, c) {
        if (0 > a || a >= this.frameCount || !this.intraFrames.length) return !1;
        for (var b = null, f = 0; f < this.intraFrames.length && this.intraFrames[f].frame <= a; f++) b = this.intraFrames[f];
        this.buffer.index = b.index;
        this.currentFrame = b.frame - 1;
        if (c) {
            for (b = b.frame; b < a; b++) this.decodePicture(D), this.findStartCode(x);
            this.currentFrame = a - 1
        }
        this.decodePicture();
        return !0
    };
    d.prototype.seekToTime = function (a, c) {
        this.seekToFrame(a * this.pictureRate | 0, c)
    };
    d.prototype.play = function () {
        this.playing || (this.targetTime = this.now(), this.wantsToPlay = this.playing = !0, this.scheduleNextFrame())
    };
    d.prototype.pause = function () {
        this.wantsToPlay = this.playing = !1
    };
    d.prototype.stop = function () {
        this.currentFrame = -1;
        this.buffer && (this.buffer.index = this.firstSequenceHeader);
        this.playing = !1;
        this.client && (this.client.close(), this.client = null);
        this.wantsToPlay = !1
    };
    d.prototype.readCode = function (a) {
        var c = 0;
        do c = a[c + this.buffer.getBits(1)]; while (0 <= c && 0 !== a[c]);
        return a[c + 2]
    };
    d.prototype.findStartCode = function (a) {
        for (var c = 0; ;)
            if (c = this.buffer.findNextMPEGStartCode(), c === a || c === v.NOT_FOUND) return c
    };
    d.prototype.fillArray = function (a, c) {
        for (var b = 0, f = a.length; b < f; b++) a[b] = c
    };
    d.prototype.pictureRate = 30;
    d.prototype.lateTime = 0;
    d.prototype.firstSequenceHeader = 0;
    d.prototype.targetTime = 0;
    d.prototype.benchmark = !1;
    d.prototype.benchFrame = 0;
    d.prototype.benchDecodeTimes = 0;
    d.prototype.benchAvgFrameTime = 0;
    ...
    d.prototype.nextFrame = function () {
        if (this.buffer)
            for (var a = this.now(); ;) {
                var c = this.buffer.findNextMPEGStartCode();
                if (c === E) this.decodeSequenceHeader();
                else {
                    if (c === x) {
                        ...
                        this.playing && this.scheduleNextFrame();
                        this.decodePicture();
                        this.benchDecodeTimes += this.now() - a;
                        return this.canvas
                    }
                    if (c === v.NOT_FOUND) return this.stop(), this.externalFinishedCallback && this.externalFinishedCallback(this), this.loop && this.sequenceStarted && this.play(), null
                }
            }
    };
    d.prototype.scheduleNextFrame = function () {
        this.lateTime = this.now() - this.targetTime;
        var a = Math.max(0, 1E3 / this.pictureRate - this.lateTime);
        this.targetTime = this.now() + a;
        this.benchmark ? (this.benchFrame++, 120 <= this.benchFrame && (this.benchAvgFrameTime = this.benchDecodeTimes / this.benchFrame, this.benchDecodeTimes = this.benchFrame = 0, w.console && console.log("Average time per frame:", this.benchAvgFrameTime, "ms")), setTimeout(this.nextFrame.bind(this), 0)) : 18 > a ? this.scheduleAnimation() : setTimeout(this.scheduleAnimation.bind(this), a)
    };
    d.prototype.scheduleAnimation = function () {
        H(this.nextFrame.bind(this), this.canvas)
    };
    d.prototype.decodeSequenceHeader = function () {
        this.width = this.buffer.getBits(12);
        this.height = this.buffer.getBits(12);
        this.buffer.advance(4);
        this.pictureRate = K[this.buffer.getBits(4)];
        this.buffer.advance(30);
        this.initBuffers();
        var a;
        if (this.buffer.getBits(1)) {
            for (a = 0; 64 > a; a++) this.customIntraQuantMatrix[F[a]] = this.buffer.getBits(8);
            this.intraQuantMatrix = this.customIntraQuantMatrix
        }
        if (this.buffer.getBits(1)) {
            for (a = 0; 64 > a; a++) this.customNonIntraQuantMatrix[F[a]] = this.buffer.getBits(8);
            this.nonIntraQuantMatrix = this.customNonIntraQuantMatrix
        }
    };
    d.prototype.initBuffers = function () {
        this.intraQuantMatrix = L;
        this.nonIntraQuantMatrix = M;
        ...
        if (!this.sequenceStarted) {
            ...
            var a = w.Uint8ClampedArray || w.Uint8Array;
            w.Uint8ClampedArray || (this.copyBlockToDestination = this.copyBlockToDestinationClamp, this.addBlockToDestination = this.addBlockToDestinationClamp);
            ...
            this.gl ? (this.gl.useProgram(this.program), this.gl.viewport(0, 0, this.width, this.height)) : (this.currentRGBA = this.canvasContext.getImageData(0, 0, this.width, this.height), this.fillArray(this.currentRGBA.data, 255))
        }
    };
    d.prototype.currentY = null;
    d.prototype.currentCr = null;
    d.prototype.currentCb = null;
    d.prototype.currentRGBA = null;
    d.prototype.pictureCodingType = 0;
    d.prototype.forwardY = null;
    d.prototype.forwardCr = null;
    d.prototype.forwardCb = null;
    d.prototype.fullPelForward = !1;
    d.prototype.forwardFCode = 0;
    d.prototype.forwardRSize = 0;
    d.prototype.forwardF = 0;
    d.prototype.decodePicture = function (a) {
        this.currentFrame++;
        ...
        if (!(0 >= this.pictureCodingType || this.pictureCodingType >= N)) {
            if (this.pictureCodingType === C) {
                this.fullPelForward = this.buffer.getBits(1);
                this.forwardFCode = this.buffer.getBits(3);
                if (0 === this.forwardFCode) return;
                this.forwardRSize = this.forwardFCode - 1;
                this.forwardF = 1 << this.forwardRSize
            }
            var c = 0;
            do c = this.buffer.findNextMPEGStartCode(); while (c === O || c === P);
            for (; c >= Q && c <= R;) this.decodeSlice(c & 255), c = this.buffer.findNextMPEGStartCode();
            ...
            if (this.pictureCodingType === B || this.pictureCodingType === C) {
                a = this.forwardY;
                var c = this.forwardY32,
                    b = this.forwardCr,
                    f = this.forwardCr32,
                    d = this.forwardCb,
                    h = this.forwardCb32;
    ...
    d.prototype.YCbCrToRGBA = function () {
        for (var a = this.currentY, c = this.currentCb, b = this.currentCr, f = this.currentRGBA.data, d = 0, h = this.codedWidth, p = this.codedWidth + (this.codedWidth - this.width), l = 0, e = this.halfWidth - (this.width >> 1), k = 0, u = 4 * this.width, g = 4 * this.width, n = this.width >> 1, v = this.height >> 1, t, m, r, w = 0; w < v; w++) {
            for (var x = 0; x < n; x++) {
                t = c[l];
                m = b[l];
                l++;
                r = m + (103 * m >> 8) - 179;
                m = (88 * t >> 8) - 44 + (183 * m >> 8) - 91;
                t = t + (198 * t >> 8) - 227;
                var y = a[d++],
                    z = a[d++];
                f[k] = y + r;
    ...
    d.prototype.renderFrame2D = function () {
        this.YCbCrToRGBA();
        this.canvasContext.putImageData(this.currentRGBA, 0, 0)
    };
    d.prototype.gl = null;
    d.prototype.program = null;
    d.prototype.YTexture = null;
    d.prototype.CBTexture = null;
    d.prototype.CRTexture = null;
    d.prototype.createTexture = function (a, c) {
        var b = this.gl,
            f = b.createTexture();
        ...
        b.uniform1i(b.getUniformLocation(this.program, c), a);
        return f
    };
    d.prototype.compileShader = function (a, c) {
        ...
        b.shaderSource(f, c);
        b.compileShader(f);
        if (!b.getShaderParameter(f, b.COMPILE_STATUS)) throw Error(b.getShaderInfoLog(f));
        return f
    };
    d.prototype.initWebGL = function () {
        var a;
        try {
        ...
        a.attachShader(this.program, this.compileShader(a.VERTEX_SHADER, I));
        a.attachShader(this.program, this.compileShader(a.FRAGMENT_SHADER, S));
        a.linkProgram(this.program);
        if (!a.getProgramParameter(this.program, a.LINK_STATUS)) throw Error(a.getProgramInfoLog(this.program));
        ...
        a.attachShader(this.loadingProgram, this.compileShader(a.VERTEX_SHADER, I));
        a.attachShader(this.loadingProgram, this.compileShader(a.FRAGMENT_SHADER, T));
        a.linkProgram(this.loadingProgram);
        a.useProgram(this.loadingProgram);
        c = a.getAttribLocation(this.loadingProgram, "vertex");
        a.enableVertexAttribArray(c);
        a.vertexAttribPointer(c, 2, a.FLOAT, !1, 0, 0);
        return !0
    };
    d.prototype.renderFrameGL = function () {
        var a = this.gl,
            c = new Uint8Array(this.currentY.buffer),
            b = new Uint8Array(this.currentCr.buffer),
            f = new Uint8Array(this.currentCb.buffer);
        a.activeTexture(a.TEXTURE0);
        a.bindTexture(a.TEXTURE_2D, this.YTexture);
        a.texImage2D(a.TEXTURE_2D, 0, a.LUMINANCE, this.codedWidth, this.height, 0, a.LUMINANCE, a.UNSIGNED_BYTE, c);
        a.activeTexture(a.TEXTURE1);
        a.bindTexture(a.TEXTURE_2D, this.CBTexture);
        a.texImage2D(a.TEXTURE_2D, 0, a.LUMINANCE, this.halfWidth, this.height / 2, 0, a.LUMINANCE, a.UNSIGNED_BYTE, b);
        a.activeTexture(a.TEXTURE2);
        a.bindTexture(a.TEXTURE_2D, this.CRTexture);
        a.texImage2D(a.TEXTURE_2D, 0, a.LUMINANCE, this.halfWidth, this.height / 2, 0, a.LUMINANCE, a.UNSIGNED_BYTE, f);
        a.drawArrays(a.TRIANGLE_STRIP, 0, 4)
    };
    d.prototype.quantizerScale = 0;
    d.prototype.sliceBegin = !1;
    d.prototype.decodeSlice = function (a) {
        this.sliceBegin = !0;
        this.macroblockAddress = (a - 1) * this.mbWidth - 1;
        this.motionFwV = this.motionFwVPrev = this.motionFwH = this.motionFwHPrev = 0;
        this.dcPredictorCb = this.dcPredictorCr = this.dcPredictorY = 128;
        for (this.quantizerScale = this.buffer.getBits(5); this.buffer.getBits(1);) this.buffer.advance(8);
        do this.decodeMacroblock(); while (!this.buffer.nextBytesAreStartCode())
    };
    d.prototype.macroblockAddress = 0;
    d.prototype.mbRow = 0;
    d.prototype.mbCol = 0;
    d.prototype.macroblockType = 0;
    d.prototype.macroblockIntra = !1;
    d.prototype.macroblockMotFw = !1;
    d.prototype.motionFwH = 0;
    d.prototype.motionFwV = 0;
    d.prototype.motionFwHPrev = 0;
    d.prototype.motionFwVPrev = 0;
    d.prototype.decodeMacroblock = function () {
        for (var a = 0, c = this.readCode(G); 34 === c;) c = this.readCode(G);
        for (; 35 === c;) a += 33, c = this.readCode(G);
        a += c;
        if (this.sliceBegin) this.sliceBegin = !1, this.macroblockAddress += a;
        else {
            if (this.macroblockAddress + a >= this.mbSize) return;
            1 < a && (this.dcPredictorCb = this.dcPredictorCr = this.dcPredictorY = 128, this.pictureCodingType === C && (this.motionFwV = this.motionFwVPrev = this.motionFwH = this.motionFwHPrev = 0));
            for (; 1 < a;) this.macroblockAddress++, this.mbRow = this.macroblockAddress / this.mbWidth | 0, this.mbCol = this.macroblockAddress % this.mbWidth, this.copyMacroblock(this.motionFwH, this.motionFwV, this.forwardY, this.forwardCr, this.forwardCb), a--;
            this.macroblockAddress++
        }
        this.mbRow = this.macroblockAddress / this.mbWidth | 0;
        this.mbCol = this.macroblockAddress % this.mbWidth;
        this.macroblockType = this.readCode(U[this.pictureCodingType]);
        this.macroblockIntra = this.macroblockType & 1;
        this.macroblockMotFw = this.macroblockType & 8;
        0 !== (this.macroblockType & 16) && (this.quantizerScale = this.buffer.getBits(5));
        this.macroblockIntra ? this.motionFwV = this.motionFwVPrev = this.motionFwH = this.motionFwHPrev = 0 : (this.dcPredictorCb = this.dcPredictorCr = this.dcPredictorY = 128, this.decodeMotionVectors(), this.copyMacroblock(this.motionFwH, this.motionFwV, this.forwardY, this.forwardCr, this.forwardCb));
        for (var a = 0 !== (this.macroblockType & 2) ? this.readCode(V) : this.macroblockIntra ? 63 : 0, c = 0, b = 32; 6 > c; c++) 0 !== (a & b) && this.decodeBlock(c), b >>= 1
    };
    d.prototype.decodeMotionVectors = function () {
        var a, c;
        c = 0;
        this.macroblockMotFw ? (a = this.readCode(J), 0 !== a && 1 !== this.forwardF ? (c = this.buffer.getBits(this.forwardRSize), c = (Math.abs(a) - 1 << this.forwardRSize) + c + 1, 0 > a && (c = -c)) : c = a, this.motionFwHPrev += c, this.motionFwHPrev > (this.forwardF << 4) - 1 ? this.motionFwHPrev -= this.forwardF << 5 : this.motionFwHPrev < -this.forwardF << 4 && (this.motionFwHPrev += this.forwardF << 5), this.motionFwH = this.motionFwHPrev, this.fullPelForward && (this.motionFwH <<= 1), a = this.readCode(J), 0 !== a && 1 !== this.forwardF ? (c = this.buffer.getBits(this.forwardRSize), c = (Math.abs(a) - 1 << this.forwardRSize) + c + 1, 0 > a && (c = -c)) : c = a, this.motionFwVPrev += c, this.motionFwVPrev > (this.forwardF << 4) - 1 ? this.motionFwVPrev -= this.forwardF << 5 : this.motionFwVPrev < -this.forwardF << 4 && (this.motionFwVPrev += this.forwardF << 5), this.motionFwV = this.motionFwVPrev, this.fullPelForward && (this.motionFwV <<= 1)) : this.pictureCodingType === C && (this.motionFwV = this.motionFwVPrev = this.motionFwH = this.motionFwHPrev = 0)
    };
    d.prototype.copyMacroblock = function (a, c, b, f, d) {
        var h, p, l, e, k, u, g = this.currentY32,
            n = this.currentCb32,
            v = this.currentCr32;
        h = this.codedWidth;
        p = h - 16;
        l = 1 === (c & 1);
        e = ((this.mbRow << 4) + (c >> 1)) * h + (this.mbCol << 4) + (a >> 1);
        k = this.mbRow * h + this.mbCol << 2;
        u = k + (h << 2);
        var t, m, r;
        if (1 === (a & 1))
            if (l)
                for (; k < u;) {
                    t = b[e] + b[e + h];
                    e++;
                    for (l = 0; 4 > l; l++) m = b[e] + b[e + h], e++, r = t + m + 2 >> 2 & 255, t = b[e] + b[e + h], e++, r |= t + m + 2 << 6 & 65280, m = b[e] + b[e + h], e++, r |= t + m + 2 << 14 & 16711680, t = b[e] + b[e + h], e++, r |= t + m + 2 << 22 & 4278190080, g[k++] = r;
                    k += p >> 2;
                    e += p - 1
                } else
                for (; k < u;) {
                    t = b[e++];
                    for (l = 0; 4 > l; l++) m = b[e++], r = t + m + 1 >> 1 & 255, t = b[e++], r |= t + m + 1 << 7 & 65280, m = b[e++], r |= t + m + 1 << 15 & 16711680, t = b[e++], r |= t + m + 1 << 23 & 4278190080, g[k++] = r;
                    k += p >> 2;
                    e += p - 1
                } else if (l)
            for (; k < u;) {
                for (l = 0; 4 > l; l++) r = b[e] + b[e + h] + 1 >> 1 & 255, e++, r |= b[e] + b[e + h] + 1 << 7 & 65280, e++, r |= b[e] + b[e + h] + 1 << 15 & 16711680, e++, r |= b[e] + b[e + h] + 1 << 23 & 4278190080, e++, g[k++] = r;
                k += p >> 2;
                e += p
            } else
            for (; k < u;) {
                for (l = 0; 4 > l; l++) r = b[e], e++, r |= b[e] << 8, e++, r |= b[e] << 16, e++, r |= b[e] << 24, e++, g[k++] = r;
                k += p >> 2;
                e += p
            }
        h = this.halfWidth;
        p = h - 8;
        l = 1 === (c / 2 & 1);
        e = ((this.mbRow << 3) + (c / 2 >> 1)) * h + (this.mbCol << 3) + (a / 2 >> 1);
        k = this.mbRow * h + this.mbCol << 1;
        u = k + (h << 1);
        if (1 === (a / 2 & 1))
            if (l)
                for (; k < u;) {
                    ...
                    g = d[e] + d[e + h];
                    e++;
                    for (l = 0; 2 > l; l++) c = f[e] + f[e + h], t = d[e] + d[e + h], e++, b = a + c + 2 >> 2 & 255, m = g + t + 2 >> 2 & 255, a = f[e] + f[e + h], g = d[e] + d[e + h], e++, b |= a + c + 2 << 6 & 65280, m |= g + t + 2 << 6 & 65280, c = f[e] + f[e + h], t = d[e] + d[e + h], e++, b |= a + c + 2 << 14 & 16711680, m |= g + t + 2 << 14 & 16711680, a = f[e] + f[e + h], g = d[e] + d[e + h], e++, b |= a + c + 2 << 22 & 4278190080, m |= g + t + 2 << 22 & 4278190080, v[k] = b, n[k] = m, k++;
                    k += p >> 2;
                    e += p - 1
                } else
                for (; k < u;) {
                    ...
                    g = d[e];
                    e++;
                    for (l = 0; 2 > l; l++) c = f[e], t = d[e++], b = a + c + 1 >> 1 & 255, m = g + t + 1 >> 1 & 255, a = f[e], g = d[e++], b |= a + c + 1 << 7 & 65280, m |= g + t + 1 << 7 & 65280, c = f[e], t = d[e++], b |= a + c + 1 << 15 & 16711680, m |= g + t + 1 << 15 & 16711680, a = f[e], g = d[e++], b |= a + c + 1 << 23 & 4278190080, m |= g + t + 1 << 23 & 4278190080, v[k] = b, n[k] = m, k++;
                    k += p >> 2;
                    e += p - 1
                } else if (l)
            for (; k < u;) {
                for (l = 0; 2 > l; l++) b = f[e] + f[e + h] + 1 >> 1 & 255, m = d[e] + d[e + h] + 1 >> 1 & 255, e++, b |= f[e] + f[e + h] + 1 << 7 & 65280, m |= d[e] + d[e + h] + 1 << 7 & 65280, e++, b |= f[e] + f[e + h] + 1 << 15 & 16711680, m |= d[e] + d[e + h] + 1 << 15 & 16711680, e++, b |= f[e] + f[e + h] + 1 << 23 & 4278190080, m |= d[e] + d[e + h] + 1 << 23 & 4278190080, e++, v[k] = b, n[k] = m, k++;
                k += p >> 2;
                e += p
            } else
            for (; k < u;) {
                for (l = 0; 2 > l; l++) b = f[e], m = d[e], e++, b |= f[e] << 8, m |= d[e] << 8, e++, b |= f[e] << 16, m |= d[e] << 16, e++, b |= f[e] << 24, m |= d[e] << 24, e++, v[k] = b, n[k] = m, k++;
                k += p >> 2;
                e += p
            }
    };
    d.prototype.blockData = null;
    d.prototype.decodeBlock = function (a) {
        var c = 0,
            b;
        if (this.macroblockIntra) {
            4 > a ? (c = this.dcPredictorY, b = this.readCode(W)) : (c = 4 === a ? this.dcPredictorCr : this.dcPredictorCb, b = this.readCode(X));
            if (0 < b) {
                var f = this.buffer.getBits(b);
                this.blockData[0] = 0 !== (f & 1 << b - 1) ? c + f : c + (-1 << b | f + 1)
            } else this.blockData[0] = c;
            4 > a ? this.dcPredictorY = this.blockData[0] : 4 === a ? this.dcPredictorCr = this.blockData[0] : this.dcPredictorCb = this.blockData[0];
            this.blockData[0] <<= 8;
            b = this.intraQuantMatrix;
            c = 1
        } else b = this.nonIntraQuantMatrix;
        for (f = 0; ;) {
            var d = 0,
                f = this.readCode(Y);
            if (1 === f && 0 < c && 0 === this.buffer.getBits(1)) break;
            65535 === f ? (d = this.buffer.getBits(6), f = this.buffer.getBits(8), 0 === f ? f = this.buffer.getBits(8) : 128 === f ? f = this.buffer.getBits(8) - 256 : 128 < f && (f -= 256)) : (d = f >> 8, f &= 255, this.buffer.getBits(1) && (f = -f));
            c += d;
            d = F[c];
            c++;
            f <<= 1;
            this.macroblockIntra || (f += 0 > f ? -1 : 1);
            f = f * this.quantizerScale * b[d] >> 4;
            0 === (f & 1) && (f -= 0 < f ? 1 : -1);
            2047 < f ? f = 2047 : -2048 > f && (f = -2048);
            this.blockData[d] = f * Z[d]
        }
        4 > a ? (b = this.currentY, d = this.codedWidth - 8, f = this.mbRow * this.codedWidth + this.mbCol << 4, 0 !== (a & 1) && (f += 8), 0 !== (a & 2) && (f += this.codedWidth << 3)) : (b = 4 === a ? this.currentCb : this.currentCr, d = (this.codedWidth >> 1) - 8, f = (this.mbRow * this.codedWidth << 2) + (this.mbCol << 3));
        this.macroblockIntra ? 1 === c ? (this.copyValueToDestination(this.blockData[0] + 128 >> 8, b, f, d), this.blockData[0] = 0) : (this.IDCT(), this.copyBlockToDestination(this.blockData, b, f, d), this.blockData.set(this.zeroBlockData)) : 1 === c ? (this.addValueToDestination(this.blockData[0] + 128 >> 8, b, f, d), this.blockData[0] = 0) : (this.IDCT(), this.addBlockToDestination(this.blockData, b, f, d), this.blockData.set(this.zeroBlockData))
    };
    d.prototype.copyBlockToDestination = function (a, c, b, d) {
        for (var q = 0; 64 > q; q += 8, b += d + 8) c[b + 0] = a[q + 0], c[b + 1] = a[q + 1], c[b + 2] = a[q + 2], c[b + 3] = a[q + 3], c[b + 4] = a[q + 4], c[b + 5] = a[q + 5], c[b + 6] = a[q + 6], c[b + 7] = a[q + 7]
    };
    d.prototype.addBlockToDestination = function (a, c, b, d) {
        for (var q = 0; 64 > q; q += 8, b += d + 8) c[b + 0] += a[q + 0], c[b + 1] += a[q + 1], c[b + 2] += a[q + 2], c[b + 3] += a[q + 3], c[b + 4] += a[q + 4], c[b + 5] += a[q + 5], c[b + 6] += a[q + 6], c[b + 7] += a[q + 7]
    };
    d.prototype.copyValueToDestination = function (a, c, b, d) {
        for (var q = 0; 64 > q; q += 8, b += d + 8) c[b + 0] = a, c[b + 1] = a, c[b + 2] = a, c[b + 3] = a, c[b + 4] = a, c[b + 5] = a, c[b + 6] = a, c[b + 7] = a
    };
    d.prototype.addValueToDestination = function (a, c, b, d) {
        for (var q = 0; 64 > q; q += 8, b += d + 8) c[b + 0] += a, c[b + 1] += a, c[b + 2] += a, c[b + 3] += a, c[b + 4] += a, c[b + 5] += a, c[b + 6] += a, c[b + 7] += a
    };
    d.prototype.copyBlockToDestinationClamp = function (a, c, b, d) {
        for (var q = 0, h = 0; 8 > h; h++) {
            for (var p = 0; 8 > p; p++) {
                var l = a[q++];
                c[b++] = 255 < l ? 255 : 0 > l ? 0 : l
            }
            b += d
        }
    };
    d.prototype.addBlockToDestinationClamp = function (a, c, b, d) {
        for (var q = 0, h = 0; 8 > h; h++) {
            for (var p = 0; 8 > p; p++) {
                var l = a[q++] + c[b];
                c[b++] = 255 < l ? 255 : 0 > l ? 0 : l
    ...
    d.prototype.IDCT = function () {
        var a, c, b, d, q, h, p, l, e, k, u, g, n = this.blockData;
        for (g = 0; 8 > g; ++g) a = n[32 + g], c = n[16 + g] + n[48 + g], b = n[40 + g] - n[24 + g], h = n[8 + g] + n[56 + g], p = n[24 + g] + n[40 + g], d = n[8 + g] - n[56 + g], q = h + p, l = n[0 + g], u = (473 * d - 196 * b + 128 >> 8) - q, h = u - (362 * (h - p) + 128 >> 8), p = l - a, e = (362 * (n[16 + g] - n[48 + g]) + 128 >> 8) - c, k = l + a, a = p + e, l = k + c, p -= e, c = k - c, b = -h - (473 * b + 196 * d + 128 >> 8), n[0 + g] = q + l, n[8 + g] = u + a, n[16 + g] = p - h, n[24 + g] = c - b, n[32 + g] = c + b, n[40 + g] = h + p, n[48 + g] = a - u, n[56 + g] = l - q;
        for (g = 0; 64 > g; g += 8) a = n[4 + g], c = n[2 + g] + n[6 + g], b = n[5 + g] - n[3 + g], h = n[1 + g] + n[7 + g], p = n[3 + g] + n[5 + g], d = n[1 + g] - n[7 + g], q = h + p, l = n[0 + g], u = (473 * d - 196 * b + 128 >> 8) - q, h = u - (362 * (h - p) + 128 >> 8), p = l - a, e = (362 * (n[2 + g] - n[6 + g]) + 128 >> 8) - c, k = l + a, a = p + e, l = k + c, p -= e, c = k - c, b = -h - (473 * b + 196 * d + 128 >> 8), n[0 + g] = q + l + 128 >> 8, n[1 + g] = u + a + 128 >> 8, n[2 + g] = p - h + 128 >> 8, n[3 + g] = c - b + 128 >> 8, n[4 + g] = c + b + 128 >> 8, n[5 + g] = h + p + 128 >> 8, n[6 + g] = a - u + 128 >> 8, n[7 + g] = l - q + 128 >> 8
    };
    var A = "jsmp",
        D = 1,
        K = [0, 23.976, 24, 25, 29.97, 30, 50, 59.94, 60, 0, 0, 0, 0, 0, 0, 0],
        F = new Uint8Array([0, 1, 8, 16, 9, 2, 3, 10, 17, 24, 32, 25, 18, 11, 4, 5, 12, 19, 26, 33, 40, 48, 41, 34, 27, 20, 13, 6, 7, 14, 21, 28, 35, 42, 49, 56, 57, 50, 43, 36, 29, 22, 15, 23, 30, 37, 44, 51, 58, 59, 52, 45, 38, 31, 39, 46, 53, 60, 61, 54, 47, 55, 62, 63]),
        L = new Uint8Array([8, 16, 19, 22, 26, 27, 29, 34, 16, 16, 22, 24, 27, 29, 34, 37, 19, 22, 26, 27, 29, 34, 34, 38, 22, 22, 26, 27, 29, 34, 37, 40, 22, 26, 27, 29, 32, 35, 40, 48, 26, 27, 29, 32, 35, 40, 48, 58, 26, 27, 29, 34, 38, 46, 56, 69, 27, 29, 35, 38, 46, 56, 69, 83]),
        M = new Uint8Array([16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16]),
        Z = new Uint8Array([32, 44, 42, 38, 32, 25, 17, 9, 44, 62, 58, 52, 44, 35, 24, 12, 42, 58, 55, 49, 42, 33, 23, 12, 38, 52, 49, 44, 38, 30, 20, 10, 32, 44, 42, 38, 32, 25, 17, 9, 25, 35, 33, 30, 25, 20, 14, 7, 17, 24, 23, 20, 17, 14, 9, 5, 9, 12, 12, 10, 9, 7, 5, 2]),
        G = new Int16Array([3, 6, 0, 9, 12, 0, 0, 0, 1, 15, 18, 0, 21, 24, 0, 27, 30, 0, 33, 36, 0, 0, 0, 3, 0, 0, 2, 39, 42, 0, 45, 48, 0, 0, 0, 5, 0, 0, 4, 51, 54, 0, 57, 60, 0, 0, 0, 7, 0, 0, 6, 63, 66, 0, 69, 72, 0, 75, 78, 0, 81, 84, 0, -1, 87, 0, -1, 90, 0, 93, 96, 0, 99, 102, 0, 105, 108, 0, 111, 114, 0, 0, 0, 9, 0, 0, 8, 117, 120, 0, 123, 126, 0, 129, 132, 0, 135, 138, 0, 0, 0, 15, 0, 0, 14, 0, 0, 13, 0, 0, 12, 0, 0, 11, 0, 0, 10, 141, -1, 0, -1, 144, 0, 147, 150, 0, 153, 156, 0, 159, 162, 0, 165, 168, 0, 171, 174, 0, 177, 180, 0, 183, -1, 0, -1, 186, 0, 189, 192, 0, 195, 198, 0, 201, 204, 0, 207, 210, 0, 213, 216, 0, 219, 222, 0, 0, 0, 21, 0, 0, 20, 0, 0, 19, 0, 0, 18, 0, 0, 17, 0, 0, 16, 0, 0, 35, 0, 0, 34, 0, 0, 33, 0, 0, 32, 0, 0, 31, 0, 0, 30, 0, 0, 29, 0, 0, 28, 0, 0, 27, 0, 0, 26, 0, 0, 25, 0, 0, 24, 0, 0, 23, 0, 0, 22]),
        d = new Int8Array([3, 6, 0, -1, 9, 0, 0, 0, 1, 0, 0, 17]),
        aa = new Int8Array([3, 6, 0, 9, 12, 0, 0, 0, 10, 15, 18, 0, 0, 0, 2, 21, 24, 0, 0, 0, 8, 27, 30, 0, 33, 36, 0, -1, 39, 0, 0, 0, 18, 0, 0, 26, 0, 0, 1, 0, 0, 17]),
        ba = new Int8Array([3, 6, 0, 9, 15, 0, 12, 18, 0, 24, 21, 0, 0, 0, 12, 27, 30, 0, 0, 0, 14, 39, 42, 0, 36, 33, 0, 0, 0, 4, 0, 0, 6, 54, 48, 0, 45, 51, 0, 0, 0, 8, 0, 0, 10, -1, 57, 0, 0, 0, 1, 60, 63, 0, 0, 0, 30, 0, 0, 17, 0, 0, 22, 0, 0, 26]),
        ...
        v = function (a) {
            ...
            this.writePos = this.length = this.bytes.length;
            this.index = 0
        };
    v.NOT_FOUND = -1;
    v.prototype.findNextMPEGStartCode = function () {
    ...
    v.prototype.nextBytesAreStartCode = function () {
        var a = this.index + 7 >> 3;
        return a >= this.writePos || 0 === this.bytes[a] && 0 === this.bytes[a + 1] && 1 === this.bytes[a + 2]
    };
    v.prototype.nextBits = function (a) {
        var c = this.index >> 3,
            b = 8 - this.index % 8;
        if (b >= a) return this.bytes[c] >> b - a & 255 >> 8 - a;
        var d = (this.index + a) % 8;
        a = this.index + a - 1 >> 3;
        b = this.bytes[c] & 255 >> 8 - b;
        for (c++; c < a; c++) b <<= 8, b |= this.bytes[c];
        0 < d ? b = b << d | this.bytes[c] >> 8 - d : (b <<= 8, b |= this.bytes[c]);
        return b
    };
    v.prototype.getBits = function (a) {
        var c = this.nextBits(a);
        this.index += a;
        return c
    };
    v.prototype.advance = function (a) {
        return this.index += a
    };
    v.prototype.rewind = function (a) {
        return this.index -= a
    }
})(window);
...
function AiActivAppsMobileAutoPlayVideo(containerID, videoID, options, videoScreenCapImgSrc, canvasOptions, videouioptions) {
    ...
    function initClass() {
        aiactivAppsMobileVideoFallbackHelper();
        ...
        if (isNativeVideoSupported() === true) {
            ...
            fallbackPlayer = new AiActivAppsVideoPreview(containerID, videoID, extraOptions, videoScreenCapImgSrc, canvasOptions, videouioptions);
        }
    };

    function checkCreativeRequirement() {
        if (aiactivAppsMobileVideoElementGenerator === null) {
            errorLog('generateMaterial or aiactivAppsMaterialGenerator function didn\'t exist.');
            return false;
        }

        if (typeof (AiActivHTMLAd) !== 'object') {
            errorLog('AiActivHTMLAd object from Advenue didn\'t exist.');
            return false;
        }

        if (typeof (Browser) !== 'function' && typeof (MobilePlatform) !== 'function' &&
            typeof AiActivAppsMobilePlatform !== 'function') {
    ...
    function platformDetectorWrapper() {
        ...
        } else if (typeof AiActivAppsMobilePlatform === 'function') {
            platform = new AiActivAppsMobilePlatform();
        }
    };
...
function MobileNativeVideo(container, videoID, extraOptions, platform) {
    ...
    function initClass() {
        ...
        if (extraOptions.tracking === true) {
            AiActivHTMLAd.attachVideo(mutedVideoEl);
        ...
        return Boolean(_video.mozHasAudio) ||
            ...
            (function () {
                if (_video.webkitAudioDecodedByteCount) {
                    if (_video.webkitAudioDecodedByteCount > 66) {
                        return true;
                    }
                }
                return false;
    ...
    function VideoControllerUI(clickCallback, enterFullScreenCallback, toggleSoundCallback) {
        ...
        function generateControlComponent() {
            // Controllers
            var controllerEl = [{
                cssClass: ['top-vidcta-container', 'full-video-height'],
                cssStyle: {
                    opacity: 1
                },
                clickFunc: clickOnCTA_,
                childs: [{
                    cssClass: ['text-vidcta-wrapper'],
                    childs: [{
                        elType: 'span',
                        cssClass: ['cta-text'],
                        innerHTML: extraOptions.playstatectatext
                    },
                    {
                        elType: 'span',
                        innerHTML: videoSvg.landing()
                    }
                    ]
                }]
            },
            ...
            {
                id: 'play_btn_when_paused_' + videoID,
                cssClass: ['status-container'],
                cssStyle: {
                    display: 'none'
                },
                childs: [{
                    cssClass: ['btn-wrapper'],
                    childs: [{
                        cssClass: ['video-round-btn'],
                        innerHTML: videoSvg.midcircle() + videoSvg.midplay()
                    }]
                }]
            }
            ];
            ...
            for (var i = 0; i < controllerEl.length; i++) {
                aiactivAppsMobileVideoElementGenerator(controllerEl[i], controlWrapperEl);
            }
            ...
            function generateControlsWrapper() {
                ...
                controlsContainer.childs.push({
                    cssClass: ['controls_wrapper', 'float-to-left'],
                    childs: [{
                        id: 'btn_sound_' + videoID,
                        cssClass: ['video-round-btn', 'audio'],
                        cssStyle: {
                            display: 'block'
                        },
                        innerHTML: videoSvg.audio()
                    },
                    {
                        id: 'text_remain_' + videoID,
                        cssClass: ['player-time', 'hidden-player-icon'],
                        innerHTML: '0:00'
                    }
                    ]
                });
                ...
                if (extraOptions.hideFullscreenIcon === false) {
                    controlsContainer.childs.push({
                        cssClass: ['controls_wrapper', 'float-to-right'],
                        childs: [{
                            id: 'btn_fs_' + videoID,
                            cssClass: ['video-round-btn', 'fs'],
                            cssStyle: {
                                display: 'block'
                            },
                            clickFunc: enterFullScreenCallback,
                            innerHTML: videoSvg.fullscreen()
                        }]
                    });
                }
        ...
        function generateEndedComponent() {
            ...
            var ctaMaterials = [{
                cssClass: ['top-vidcta-container'],
                cssStyle: {
                    opacity: 1
                },
                clickTag: extraOptions.urls,
                childs: [{
                    cssClass: ['text-vidcta-wrapper'],
                    childs: [{
                        elType: 'span',
            ...
            for (var i = 0; i < ctaMaterials.length; i++) {
                aiactivAppsMobileVideoElementGenerator(ctaMaterials[i], endedWrapperEl);
            }
        };
        ...
        function clickOnCTA_() {
            ...
            aiactivAppsTriggerClicktagHandler(extraOptions.urls);
            totalCtaClicked_++;
        }
    ...
    function PerformanceTracker(videoEl) {
        ...
        function fireLoaded() {
            AiActivHTMLAd.track(trackingVideoID + '_AutoLoaded');
        };
        ...
        function fireClickToPlay() {
            ...
            AiActivHTMLAd.track(trackingVideoID + '_ClickAutoToPlay');
            isClicked = true;
        };

        function fireEnterFullScreen() {
            if (extraOptions.tracking === true) {
                AiActivHTMLAd.track(trackingVideoID + '_OpenFullscreen');
            }
        };

        function fireExitFullScreen() {
            if (extraOptions.tracking === true) {
                AiActivHTMLAd.track(trackingVideoID + '_CloseFullscreen');
            }
        };
    };
}
...
function DesktopNativeVideo(container, videoID, extraOptions, platform) {
    ...
    var performanceTracker;
    var videoSvg = new generateVideoSVGIcon();
    var audioSvg = videoSvg.audio(),
        fullscreenSvg = videoSvg.fullscreen();
...
function AiActivAppsVideoPreview(containerID, videoID, options, videoScreenCapImgSrc, canvasOptions, videouioptions) {
    this.currentTime = 0;
    this.duration = 0;
    ...
    var videoPlayer;
    var videoUI = null;
    var canvasID = 'aiactivAppsVideoCanvas-' + videoID;
    ...
    function initClass() {
        ...
        videoUI = new AiActivAppsVideoUI(containerID, canvasID, videoID, extraOptions, videoScreenCapImgSrc, self);
    ...
    function platformDetectorWrapper() {
        if (typeof (Browser) === 'function') {
            ...
            autoplaybrowser = new AiActivAppsMobilePlatform();
        }
    }
    ...
    function getCurrentTime() {
        var c = 0;
        if (videoUI != null) c = Math.ceil(videoUI.currentTime());
        return c;
    };

    function getDuration() {
        var d = 0;
        if (videoUI != null) d = Math.ceil(videoUI.duration());
        return d;
    };
    ...
    function AiActivAppsVideoUI(containerID, canvasID, videoID, options, videoScreenCapImgSrc, aiactivAppsVideoPreview) {
        ...
        var vid, soundBtn, invisibleBtn, playBtn,
            ...
            videoprogress_interval = null,
            videoStarted = false,
            videoFinished = true;

        var slidingIntv = null,
            allowHideController = false,
            mouseAvailable = (autoplaybrowser.getOS() == 'ios' || autoplaybrowser.getOS() == 'android') ? false : true;

        videoEl = [{
            id: 'aiactivAppsLoadContainer' + customVideoID,
            cssClass: ['preview-ui-container'],
            childs: [{
                elType: 'img',
                id: 'aiactivAppsLoadBg' + customVideoID,
                cssClass: ['backupimage'],
                attrs: {
                    'src': videoScreenCapImgSrc
                }
            }]
        }];
        ...
        function initClass() {
            ...
            for (var i = 0; i < videoEl.length; i++) {
                aiactivAppsMobileVideoElementGenerator(videoEl[i], document.getElementById(containerID));
            }
            ...
            if (mobileVideo) {
                aiactivAppsVideoPreview.initVideoPlayer();
            } else {
                if (requireHTMLVideo) {
        ...
        function generateHTMLVideo() {
            ...
            // Player Status
            var playerstateEl = {
                id: 'playerstate_' + videoID,
                cssClass: ['status-container', 'preload-state', 'pause-preloader-animation'],
                childs: [{
                    id: 'btn_invisible_' + videoID,
                    cssClass: ['invisible-tap-area']
                },
                {
                    cssClass: ['btn-wrapper'],
                    childs: [{
                        id: 'btn_player_' + videoID,
                        cssClass: ['video-round-btn'],
                        innerHTML: (function () {
                            return videoSvg.midcircle() + videoSvg.fullstatus()
                        })()
                    }]
                }
                ]
            ...
            var topctaEl = {
                id: 'topcta_' + videoID,
                cssClass: ['top-vidcta-container'],
                clickTag: options.urls,
                childs: [{
                    cssClass: ['text-vidcta-wrapper'],
                    childs: [{
                        elType: 'span',
                        cssClass: ['cta-text'],
                        innerHTML: options.playstatectatext
                    },
                    {
                        elType: 'span',
                        innerHTML: videoSvg.landing()
            ...
            var controllerEl = {
                id: 'controllers_' + videoID,
                cssClass: ['controllers-container'],
                childs: [{
                    cssClass: ['controls_wrapper', 'float-to-left'],
                    childs: [{
                        id: 'btn_sound_' + videoID,
            ...
            // Advertisement
            var adLabelEl = {
                cssClass: ['advertisement-label-container'],
                clickTag: options.urls,
                childs: [{
                    elType: 'span',
                    cssClass: ['advertisement'],
                    innerHTML: options.adtext
                }]
            };

            if (options.fullscreen) {
                var _btnfs = {
                    cssClass: ['controls_wrapper', 'float-to-right', 'hidden-player-icon'],
                    childs: [{
                        id: 'btn_fs_' + videoID,
                        cssClass: ['video-round-btn', 'fs'],
                        cssStyle: {
            ...
            if (requireHTMLVideo) {
                ...
                playerstateEl = {
                    id: 'playerstate_' + videoID,
                    cssClass: ['status-container', 'preload-state', 'no-mp4'],
                    clickFunc: playHTMLVideo,
                    childs: [{
                        id: 'btn_invisible_' + videoID,
                        cssClass: ['invisible-tap-area']
                    },
            ...
            if (requireHTMLVideo) {
                if (mobileVideo) {
                    if (options.hideMpgWhenHtmlPlayed === true) {
                        ...
                        aiactivAppsVideoPreview.pausePreview();
                ...
                document.getElementById(videoID).play();
                document.getElementById('aiactivAppsLoadContainer' + customVideoID).style.display = 'none';
                performanceTracker.fireClickToPlay();
            }
        ...
        function videoIsReady() {
            ...
            if (requireHTMLVideo) {
                aiactivAppsVideoPreview.addListener("play", function () {
                    if (!videoStarted) {
                        toggleHTML5VideoStatus("nosound");
                        playerState.classList.remove("pause-preloader-animation");
                    }
                });
                aiactivAppsVideoPreview.addListener("pause", function () {
                    if (!videoStarted) {
                        playerState.classList.add("pause-preloader-animation");
                    }
                });
                aiactivAppsVideoPreview.addListener("ended", function () {
                    if (!videoStarted) {
                        toggleHTML5VideoStatus("previewend");
                    }
                });
        ...
        function trackHTML5Video() {
            if (options.tracking === true) {
                AiActivHTMLAd.attachVideo(document.getElementById(videoID));
                trackVideoID = "_Video" + AiActivHTMLAd.vids.length;
            }
        };
        ...
        function bindPlayerControllerComponent() {
            ...
            if (options.autostart && mobileVideo == false) {
                toggleHTML5VideoPlay();
            }
            if (options.advertisementlabel == false) {
                container.classList.add("no-ad-label");
            }
            ...
            if (options.fullscreen) {
                ...
                var track = {
                    openfs: (options.tracking === true) ? function () {
                        AiActivHTMLAd.track(trackVideoID + "_OpenFullscreen");
                    } : function () {
                        return;
                    },
                    closefs: (options.tracking === true) ? function () {
                        AiActivHTMLAd.track(trackVideoID + "_CloseFullscreen");
                    } : function () {
                        return;
                    }
                };
                ...
                var getPrefix = function () {
                    ...
                    function lowercase() {
                        var l = (_dom == '' || throwerror == true) ? '' : _pre;
                        return l;
                    }

                    function css() {
                        var c = (_dom == '' || throwerror == true) ? '' : '-' + _pre + '-';
                        return c;
                    }

                    function js() {
                        var j = (_dom == '' || throwerror == true) ? '' : _pre[0].toUpperCase() + _pre.substr(1);
                        return j;
                    }
                };

                try {
                    var prefix = new getPrefix(),
                        list = ["webkit", "moz", "ms", "o"];
                    if (list.indexOf(prefix.lowercase()) > -1) {
                        if (prefix.lowercase() == "ms") {
                            fsEvent = "MSFullscreenChange";
                        } else {
                            fsEvent = prefix.lowercase() + fsEvent;
                        }
                    }
                } catch (error) { }
    ...
    function PerformanceTracker(videoID) {
        ...
        function fireLoaded() {
            AiActivHTMLAd.track(trackingVideoID + '_PreviewLoaded');
        };
        ...
        function fireClickToPlay() {
            ...
            AiActivHTMLAd.track(trackingVideoID + '_ClickPreviewToPlay');
            isClicked = true;
        };
    };
};
...
/**
 * Variable to store the generateMaterial or aiactivAppsMaterialGenerator
 * function reference.
 * @type function
 */
var aiactivAppsMobileVideoElementGenerator = null;

/**
 * This function help to convert functions that are different from development
 * version to use same variable.
 */
function aiactivAppsMobileVideoFallbackHelper() {
    if (typeof generateMaterial === 'function') {
        aiactivAppsMobileVideoElementGenerator = generateMaterial
    } else if (typeof aiactivAppsMaterialGenerator === 'function') {
        aiactivAppsMobileVideoElementGenerator = aiactivAppsMaterialGenerator;
    }
}
 ...
 */
function aiactivAppsTriggerClicktagHandler(clicktag) {
    if (typeof triggerClickTag === 'function') {
        triggerClickTag(clicktag);
        return;
    }

    if (typeof aiactivAppsTriggerClickTag === 'function') {
        aiactivAppsTriggerClickTag(clicktag);
        return;
    }

    if (AiActivHTMLAd !== null && AiActivHTMLAd !== undefined) {
        if (typeof AiActivHTMLAd.click === 'function') {
            AiActivHTMLAd.click({
                clickTAG: clicktag
            });
        }
...
function AiActivAppsRevolver(opts) {
    this.setParentWindowHeight = setParentWindowHeight;
    this.shrink = shrink;

    let autoRotationDirection_ = null;
    let carolBanner_ = null;
    let curRotationIndex_ = 0;
    let extraOpts_ = mergeObject_({
        autoRotateDirection: 'clockwise',
        expandedLoadedCallback: null,
        expandedLandedCallback: null,
        invHiddenCallback: null,
        invLandedCallback: null,
    }, opts, 'Carol');
    let invCarolContainer_ = null;
    let invContainer_ = null;
    let itemBaseAngle_ = 0;
    /**
     * -1 invitation state
     * 0 expanding state
     * 1 shrinking state
     * @type {Number}
     */
    let operationState_ = -1;
    let totalItem_ = 0;
    let totalRotationBeforeExpanded_ = 0;
    let touchSceneAreaEl_ = null;
    let windowExtraHeight_ = 0;
    let parentWindowHeight_ = 0;

    initClass_();

    function initClass_() {
        if (aiactivAppsPlatform === null || aiactivAppsPlatform === undefined) {
            alert('Missing "AiActivAppsMobilePlatform.js" JS file!');
            return;
        }

        invContainer_ = document.getElementById('aiactiv-apps-invitation-ad');
        if (invContainer_ === null) {
            alert('Missing "aiactiv-apps-invitation-ad" HTML element!');
            return;
        }

        touchSceneAreaEl_ = document.getElementById('aiactiv-apps-invitation-scene');
        if (touchSceneAreaEl_ === null) {
            alert('Missing "aiactiv-apps-invitation-scene" HTML element!');
            return;
        }

        invCarolContainer_ = document.getElementById('aiactiv-apps-invitation-carol-container');
        if (invCarolContainer_ === null) {
            alert('Missing "aiactiv-apps-invitation-carol-container" HTML element!');
            return;
        }

        iosWorkaround_();

        parentWindowHeight_ = window.innerHeight;
        carolBanner_ = new CarolBanner();

        calcSmallItemPosition_();
        bindEvents_();
    ...
    function setParentWindowHeight(newWindowHeight) {
        windowExtraHeight_ = 0;
        parentWindowHeight_ = newWindowHeight;
        if (newWindowHeight < (window.innerHeight - 5)) {
            windowExtraHeight_ = (window.innerHeight - newWindowHeight) / 2;
        }

        carolBanner_.reCalcTranslateY();
    }

    function shrink() {
        invContainer_.classList.remove('aiactiv-apps-hide');
        autoRotateOneRoundReverse_();
        carolBanner_.hide();
    ...
    function autoExpand_() {
        if (carolBanner_.isExpanded() === true) {
            return;
        }

        if (Math.abs(totalRotationBeforeExpanded_) >= 2) {
            smallItemClickHandler_();
        }
    }

    function autoRotateOneRound_() {
        if (autoRotationDirection_ !== null) {
            if (autoRotationDirection_ === 'clockwise') {
                rotateRight1Round_();
            } else {
                rotateLeft1Round_();
            }

            autoRotationDirection_ = null;
            return;
        }

        if (extraOpts_.autoRotateDirection.toLowerCase() === 'clockwise') {
            rotateRight1Round_();
        } else {
            rotateLeft1Round_();
        }
    ...
    function autoRotateOneRoundReverse_() {
        if (extraOpts_.autoRotateDirection.toLowerCase() === 'clockwise') {
            rotateLeft1Round_();
        } else {
            rotateRight1Round_();
    ...
    function bindEvents_() {
        let carolTouchHandler = new CarolTouchHandler(rotateToLeft_, rotateToRight_);
        if (hasTouchScreen) {
            touchSceneAreaEl_.addEventListener('touchstart', carolTouchHandler.touchStartHandler, {
                passive: true
            });
            touchSceneAreaEl_.addEventListener('touchmove', carolTouchHandler.touchMoveHandler, {
                passive: true
            });
        } else {
            touchSceneAreaEl_.addEventListener('mousedown', carolTouchHandler.mouseDownHandler);
            touchSceneAreaEl_.addEventListener('mousemove', carolTouchHandler.mouseMoveHandler);
        }

        invContainer_.addEventListener('transitionend', invTransitionEndHandler_);

        let carolItems = document.getElementsByClassName('aiactiv-apps-carol-small');
        for (let i = 0; i < carolItems.length; i++) {
            carolItems[i].addEventListener('click', smallItemClickHandler_);
        }
    }

    function calcDesignerIndex_() {
        let tmpIndex = curRotationIndex_ % totalItem_;
        if (tmpIndex > 0) {
            tmpIndex = totalItem_ - tmpIndex + 1;
        } else if (tmpIndex < 0) {
    ...
    function calcSmallItemPosition_() {
        let carolItems = document.getElementsByClassName('aiactiv-apps-carol-small');
        if (carolItems.length < 1) {
            alert('Missing "aiactiv-apps-carol-small" HTML elements!');
            return;
        }

        let carolItemWidth = carolItems[0].clientWidth;
        totalItem_ = carolItems.length;
        itemBaseAngle_ = 360 / totalItem_;
        // - 10 is because we want to make space between item lesser.
        let carolRadius = Math.round(carolItemWidth / (2 * Math.sin(itemBaseAngle_ * Math.PI / 360))) - 10;

        for (let i = 0; i < totalItem_; i++) {
            let angle = itemBaseAngle_ * i;
            let curItem = carolItems[i];
            curItem.style.transform = `rotateY(${angle}deg) translateZ(${carolRadius}px)`;
            curItem.style.mozTransform = `rotateY(${angle}deg) translateZ(${carolRadius}px)`;
            curItem.style.webkitTransform = `rotateY(${angle}deg) translateZ(${carolRadius}px)`;
        }

        let newMarginTop = (invContainer_.clientHeight - touchSceneAreaEl_.clientHeight) / 5 * 4;
        if (aiactivAppsPlatform.getOS() === 'ios') {
            newMarginTop = (invContainer_.clientHeight - touchSceneAreaEl_.clientHeight) / 4 * 3;
        }
        touchSceneAreaEl_.style.marginTop = newMarginTop + 'px';
    }

    function invLandedToIndexCallback(isAutomation = false) {
        if (typeof extraOpts_.invLandedCallback !== 'function') {
            return;
        }

        if (operationState_ !== -1) {
            return;
        }

        extraOpts_.invLandedCallback(calcDesignerIndex_(), isAutomation);
    }

    function invTransitionEndHandler_(e) {
        if (e.target.id !== 'aiactiv-apps-invitation-ad' || e.propertyName.toLowerCase() !== 'top') {
            return;
        }
    ...
    function iosWorkaround_() {
        if (aiactivAppsPlatform.getOS() !== 'ios') {
            return;
        }

        document.getElementById('aiactiv-apps-ad').classList.add('ios-workaround');

        invCarolContainer_.classList.add('ios-workaround');
    }
    ...
    function rotate_(isAutomation = false) {
        let newAngle = itemBaseAngle_ * curRotationIndex_;
        invCarolContainer_.style.transform = `rotateY(${newAngle}deg)`;

        invLandedToIndexCallback(isAutomation);

        carolBanner_.rotate(isAutomation);
    }

    function rotateLeft1Round_() {
        curRotationIndex_ -= totalItem_;
        rotate_(true);
    }

    function rotateRight1Round_() {
        curRotationIndex_ += totalItem_;
        rotate_(true);
    }

    function rotateToLeft_() {
        totalRotationBeforeExpanded_++;
        curRotationIndex_--;
        rotate_();

        carolBanner_.loadImage();
        autoRotationDirection_ = 'anti-clockwise';
        autoExpand_();
    }

    function rotateToRight_() {
        totalRotationBeforeExpanded_++;
        curRotationIndex_++;
        rotate_();

        carolBanner_.loadImage();
        autoRotationDirection_ = 'clockwise';
        autoExpand_();
    }

    function smallItemClickHandler_(e) {
        if (e !== undefined && e.stopPropagation !== undefined && e.preventDefault !== undefined) {
            e.stopPropagation();
            e.preventDefault();
        }

        invContainer_.classList.add('aiactiv-apps-hide');
        autoRotateOneRound_();

        carolBanner_.loadImage();

        if (typeof extraOpts_.invHiddenCallback === 'function') {
            extraOpts_.invHiddenCallback();
        }
    }

    function CarolBanner() {
        this.hide = hide;
        this.isExpanded = isExpanded;
        this.loadImage = loadImage;
        this.reCalcTranslateY = reCalcTranslateY;
        this.rotate = rotate;
        this.show = show;

        let bannerAd_ = null;
        let bannerCarolContainer_ = null;
        let bannerContainerWithBg_ = null;
        let bannerSceneTouchArea_ = null;
        let carolResizeHandler_ = null;
        let isBannerLoaded_ = false;
        let isBannerExpanded_ = false;

        initClass_();

        function initClass_() {
            bannerContainerWithBg_ = document.getElementById('aiactiv-apps-expanded-bg');
            if (bannerContainerWithBg_ === null) {
                alert('Missing "aiactiv-apps-expanded-bg" HTML element!');
                return;
            }

            bannerAd_ = document.getElementById('aiactiv-apps-expanded-ad');
            if (bannerAd_ === null) {
                alert('Missing "aiactiv-apps-expanded-ad" HTML element!');
                return;
            }

            bannerCarolContainer_ = document.getElementById('aiactiv-apps-expanded-carol-container');
            if (bannerCarolContainer_ === null) {
                alert('Missing "aiactiv-apps-expanded-carol-container" HTML element!');
                return;
            }

            bannerSceneTouchArea_ = document.getElementById('aiactiv-apps-expanded-scene');
            if (bannerSceneTouchArea_ === null) {
                alert('Missing "aiactiv-apps-expanded-scene" HTML element!');
                return;
            }

            carolResizeHandler_ = new CarolBannerResizeHandler(bannerSceneTouchArea_, calcBigItemPosition_);
        ...
        function hide() {
            bannerContainerWithBg_.classList.add('aiactiv-apps-hide');
        }

        function isExpanded() {
            return isBannerExpanded_;
        }

        function loadImage() {
            if (isBannerLoaded_ === true) {
                return;
            }

            isBannerLoaded_ = true;

            let imgEls = document.getElementsByClassName('aiactiv-apps-banner-image');
            for (let i = 0; i < imgEls.length; i++) {
                let curImgEl = imgEls[i];

                curImgEl.onload = function () {
                    resizeHandler_();

                    calcBigItemPosition_();
                    // We need to call calcBigItemPosition_ again due to translateZ need
                    // sometime to render before translateY can be calculated.
                    setTimeout(calcBigItemPosition_, 1000);

                    if (typeof extraOpts_.expandedLoadedCallback === 'function') {
                        extraOpts_.expandedLoadedCallback();
                    }
                };

                curImgEl.src = curImgEl.getAttribute('data-src');
            }
        }

        function reCalcTranslateY() {
            resizeHandler_();
            calcCenterPosition_();
        }

        function rotate(isAutomation = false) {
            let newAngle = itemBaseAngle_ * curRotationIndex_;
            bannerCarolContainer_.style.transform = `rotateY(${newAngle}deg)`;

            if (operationState_ !== 0) {
                return;
            }

            if (typeof extraOpts_.expandedLandedCallback === 'function') {
                extraOpts_.expandedLandedCallback(calcDesignerIndex_(), isAutomation);
            }
        }

        function show() {
            bannerContainerWithBg_.classList.remove('aiactiv-apps-hide');
            resizeHandler_();
            isBannerExpanded_ = true;
        }
        ...
        function bindEvents_() {
            bindTouchEvents_();
            bindTransitionEvents_();
            resizeHandler_();
        }

        function bindTouchEvents_() {
            let carolTouchHandler = new CarolTouchHandler(rotateToLeft_, rotateToRight_);
            if (hasTouchScreen) {
                bannerSceneTouchArea_.addEventListener('touchstart', carolTouchHandler.touchStartHandler, {
                    passive: true
                });
                bannerSceneTouchArea_.addEventListener('touchmove', carolTouchHandler.touchMoveHandler, {
                    passive: true
                });
            } else {
                bannerSceneTouchArea_.addEventListener('mousedown', carolTouchHandler.mouseDownHandler);
                bannerSceneTouchArea_.addEventListener('mousemove', carolTouchHandler.mouseMoveHandler);
            }
        }

        function bindTransitionEvents_() {
            let carolBigEls = document.getElementsByClassName('aiactiv-apps-carol-big');
            for (let i = 0; i < carolBigEls.length; i++) {
                carolBigEls[i].addEventListener('transitionend', carolBigItemTransitionEndHandler_);
            }
        }

        function calcBigItemPosition_() {
            let carolItems = document.getElementsByClassName('aiactiv-apps-carol-big');
            if (carolItems.length < 1) {
                alert('Missing "aiactiv-apps-carol-big" HTML elements!');
                return;
            }

            let carolItemWidth = carolItems[0].clientWidth;
            totalItem_ = carolItems.length;
            let carolRadius = Math.round(carolItemWidth / (2 * Math.sin(itemBaseAngle_ * Math.PI / 360)));
        ...
        function carolBigItemTransitionEndHandler_(e) {
            if (e.target.classList.contains('aiactiv-apps-carol-big') !== true) {
                return;
            }
        ...
        function calcCenterPosition_() {
            ...
            let carolItemHeight = carolItems[calcDesignerIndex_() - 1].clientHeight;
            let carolNewY = (window.innerHeight - carolItemHeight) / 2;
            carolNewY += windowExtraHeight_;
            bannerSceneTouchArea_.style.marginTop = carolNewY + 'px';
        }

        function resizeHandler_() {
            carolResizeHandler_.onResize();

            let newTop = (window.innerHeight - bannerAd_.clientHeight) / 2;
            if (bannerAd_.clientHeight > window.innerHeight) {
                newTop = 0;
            }
            bannerAd_.style.top = newTop + 'px';

            let imgContainers = document.getElementsByClassName('aiactiv-apps-carol-big');
            for (let i = 0; i < imgContainers.length; i++) {
                let curImgContainer = imgContainers[i];
                let newHeight = curImgContainer.clientWidth / 9 * 16;
                curImgContainer.style.height = newHeight + 'px';
            }
        }
    }

    function CarolBannerResizeHandler(sceneEl, updateCarolBigPosition) {
        this.onResize = onResize;

        let heightRatio_ = 16;
        let percentageOfWindow = 0.9;
        let perspective_ = 1000;
        let widthRatio_ = 9;
        ...
        function onResize() {
            adjustItemSizeBaseOnWindowResolution_();
        }

        // Private function ======================================================

        function actualWidthFromula_(itemWidth) {
            let theta = (itemBaseAngle_ / 2) * Math.PI / 180;
            let radius = (itemWidth / 2) / Math.tan(theta);
            let zIndexScale = (perspective_ - radius) / perspective_;
            let originalWidth = Math.floor(itemWidth * zIndexScale);
            let newHeight = originalWidth / 9 * 16;

            sceneEl.style.width = originalWidth + 'px';
            sceneEl.style.height = newHeight + 'px';
        }

        function adjustItemSizeBaseOnWindowResolution_() {
            let itemWidth = window.innerWidth * percentageOfWindow;
            let itemHeight = itemWidth / widthRatio_ * heightRatio_;

            let heightDiff = parentWindowHeight_ - itemHeight;

            if (heightDiff > 0) {
                actualWidthFromula_(itemWidth);
            } else {
                useHeightAsBase_();
            }

            if (typeof updateCarolBigPosition === 'function') {
                updateCarolBigPosition();
            }
        }

        function initClass_() { }

        function useHeightAsBase_() {
            let itemHeight = parentWindowHeight_ * percentageOfWindow;
            let itemWidth = itemHeight / heightRatio_ * widthRatio_;

            actualWidthFromula_(itemWidth);
        }
    }

    function CarolTouchHandler(toLeftCallback, toRightCallback) {
        ...
        function touchMoveHandler(e) {
            ...
            let xDiff = initX_ - e.touches[0].clientX;

            if (xDiff > touchMoveThreshold) {
                toLeftCallback();
            } else if (xDiff < -touchMoveThreshold) {
                toRightCallback();
            } else {
                return;
            }
...
let aiactivAppsTimerTrackingID = null;
/**
 * For reference purpose only.
 * @type String
 */
let aiactivAppsTrackerVersion = '5.0.0';

function aiactivAppsTrackerReset() {
    aiactivAppsTriggerTimerStop();
    aiactivAppsTrackerPauseVideo();
}

function aiactivAppsTriggerClickTag(clickTag) {
    aiactivAppsTrackerPauseVideo();
    AiActivHTMLAd.click({
        clickTAG: clickTag
    });
...
function aiactivAppsTriggerTimerStart(id) {
    if (aiactivAppsTimerTrackingID === id) {
        return;
    }

    aiactivAppsTriggerTimerStop();
    AiActivHTMLAd.startTimer(id);
    aiactivAppsTimerTrackingID = id;
}

function aiactivAppsTriggerTimerStop() {
    if (aiactivAppsTimerTrackingID !== null) {
        AiActivHTMLAd.stopTimer(aiactivAppsTimerTrackingID);
        aiactivAppsTimerTrackingID = null;
    }
}

function aiactivAppsTriggerTrack(track) {
    AiActivHTMLAd.track(track);
}

function aiactivAppsTrackerPauseVideo() {
    if (typeof aiactivAppsPauseAllVideo === 'function') {
        // Helper function to pause all video.
        aiactivAppsPauseAllVideo();
    }
}

function aiactivCheckHasTouchScreen() {
