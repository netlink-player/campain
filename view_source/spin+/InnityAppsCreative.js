...
let innityAppsInvitationHeight = 0;
...
window.addEventListener('load', innityAppsInitSpinPlusBanner, false);

function innityAppsBindEvents() {
  window.addEventListener('message', innityAppsMessageHandler, false);
  innityAppsDisableEventsTravelToPublisher();
}

function innityAppsDisableEventsTravelToPublisher() {
  document.getElementById('innity-apps-ad-banner').addEventListener('touchmove', function(e) {
    e.preventDefault();
    e.stopPropagation();
  }, false);
}

function innityAppsInitSpinPlusBanner() {
  innityAppsBindEvents();
  innityAppsPostReadyToProxy();
}

function innityAppsLoadSpinBanner(sector, direction) {
  innityAppsPlatform = new InnityAppsMobilePlatform();
  innityAppsAd = new InnityAppsSpinPlusBanner();

  if (typeof invitationLandOnSlide === 'function') {
    invitationLandOnSlide(sector, direction);
  }
}
...
function innityAppsMessageHandler(event) {
  ...
  switch (supportedMsg.action) {
    case 'show':
      innityAppsInvitationHeight = supportedMsg.data.invitationHeight;
      if (innityAppsAd !== null) {
        innityAppsAd.setInvitationHeight(innityAppsInvitationHeight);
      }

      if (innityAppsIsBannerLoad === false) {
        innityAppsIsBannerLoad = true;
        innityAppsGenerateMainContent(document.getElementById('innity-apps-banner-content'));
        setTimeout(function() {
          innityAppsLoadSpinBanner(supportedMsg.data.sector, supportedMsg.data.direction);
      ...
      if (typeof invitationLandOnSlide === 'function') {
        invitationLandOnSlide(supportedMsg.data.sector, supportedMsg.data.direction);
      }
      break;
    case 'close':
      innityAppsResetBannerRotation();

      if (typeof innityAppsTrackerReset === 'function') {
      ...
      if (typeof bannerClosed === 'function') {
        bannerClosed();
      }
      break;
  }
...
function innityAppsPostMsgToProxy(action) {
  parent.postMessage({owner: 'Innity', adType: 'innity-apps-mobile-spin-banner', action: action, data: {}, version: innityAppsVersion}, '*');
}

function innityAppsPostReadyToProxy() {
  innityAppsPostMsgToProxy('adLoaded');
}

function innityAppsResetBannerRotation() {
  let bannerSlides = document.getElementsByClassName('innity-apps-banner-slide');
...
let innityAppsMaterialGeneratorVersion = '5.0.0';
...
function InnityAppsSpinPlusBanner() {
  this.onResize = onResize;
  this.setInvitationHeight = setInvitationHeight;
  ...
  function initClass() {
    adContainer = document.getElementById('innity-apps-ad');
    bannerContent = document.getElementById('innity-apps-banner-content');
  ...
  function onResize() {
    ...
    centerSafeAreaBg();
    centerSafeAreaContainer();
  }

  function centerSafeAreaBg() {
    let topSpaceHeight = Math.round(230 * (bannerContent.clientWidth / 640));
    let centerPosition = (bannerContent.clientHeight - invitationHeight - bannerContent.clientWidth) / 2;

    let safeAreaBgs = document.getElementsByClassName('innity-apps-creative-safe-area-bg');
    for (let i = 0; i < safeAreaBgs.length; i++) {
      let curSafeAreaBg = safeAreaBgs[i];
      curSafeAreaBg.style.marginTop = centerPosition - topSpaceHeight + 'px';
    }
  }

  function centerSafeAreaContainer() {
    let centerPosition = (bannerContent.clientHeight - invitationHeight - bannerContent.clientWidth) / 2;

    let safeAreaContainers = document.getElementsByClassName('innity-apps-creative-safe-area-container');
    for (let i = 0; i < safeAreaContainers.length; i++) {
      let curSafeAreaContainer = safeAreaContainers[i];
      curSafeAreaContainer.style.marginTop = centerPosition + 'px';
    }
  }

  function setInvitationHeight(height) {
    if (isNaN(height) === false) {
      invitationHeight = height;
    }
