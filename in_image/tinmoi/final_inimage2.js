var InImageTinmoiNetlink = function (intImage) {
  ...
  btnNetlinkClose.style.setProperty("right", "3px", "important");
  btnNetlinkClose.style.setProperty("top", "-25px", "important");
  btnNetlinkClose.style.display = "none";
  ...
  const intervalImg = setInterval(() => {
    if (isImageFile(images[intImage].src)) {
      ...
      const intervalId = setInterval(() => {
        if (parseInt(insElement.offsetHeight) > 0) {
          ...
          if (iframeElement) {
            ...
            if (iframeDocument) {
              ...
              if (iframeBody && iframeBody.hasChildNodes()) {
                console.log("Phần tử body trong iframe có phần tử con.");
                btnNetlinkClose.style.display = "block";
              } else {
                console.log(
                  "Phần tử body trong iframe không có phần tử con hoặc không tồn tại."
                );
